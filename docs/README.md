...
```bash
pnpm test:installer       # 144-case contract enumeration, no network or global writes
...
skill pin from the npm version. After the suite passes, start fresh Codex, Claude Code,
and Pi sessions and confirm the host-specific names in [setup](./setup.md) are visible.
Agent discovery has no stable noninteractive cross-host command, so that reload check
remains a manual release assertion.
