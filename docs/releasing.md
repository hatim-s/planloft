# Release Planloft 0.2.4

This guide publishes `planloft@0.2.4` to npm and creates the matching Git tag
`v0.2.4`. Run every step from the repository root, in the same terminal.
...
exact prepared tarball, verifies its npm SHA-1, creates `v0.2.4` on the prepared commit,
...
npm login
npm whoami
npm view planloft@0.2.4 version
...
- `npm view` returns `E404` because `0.2.4` has not been published yet.
- `git ls-remote` prints nothing because `v0.2.4` does not exist yet.
...
Expected: `git status --short` prints nothing and the version command prints `0.2.4`.
...
CANDIDATE="$RELEASE_DIR/planloft-0.2.4.tgz"
test -f "$CANDIDATE"
...
- npm creates `planloft-0.2.4.tgz`.
...
- The dry run ends with `+ planloft@0.2.4` without publishing anything.
...
Expected: npm prints `+ planloft@0.2.4`.

If the command times out or the result is unclear, do not immediately retry. Run
`npm view planloft@0.2.4 version` first to learn whether npm accepted it.

## 5. Verify npm, then create the Git tag

```bash
npm view planloft@0.2.4 version dist.shasum dist.integrity
RELEASE_COMMIT="$(git rev-parse HEAD)"
git tag -a v0.2.4 "$RELEASE_COMMIT" -m "planloft v0.2.4"
...
Expected: npm reports version `0.2.4`, the push creates `v0.2.4`, and the final lookup
shows the tag.
...
- npm result is unclear: check `npm view planloft@0.2.4 version` before retrying.
...
- Released installer assertion failed: fix the assertion on `main`, then resume at the
  ...
  `PLANLOFT_RELEASE_TAG=v0.2.4 node scripts/installer-matrix.mjs --live --full --source all --from-case-index <case>`.
