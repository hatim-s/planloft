...
across skills, README prose, and Commander help. Agent-authored plans
...
Ship only `write-doc`. Trigger it for substantial implementation, migration,
...
README and skill projections in tests.

### G4 — Author plans as Markdown

`write-doc` always writes a Markdown source with title, slug, kind, and status
...
Treat CLI-only installation and installation of only `write-doc` through the external
`skills` runner as different capability sets. Installing the skill alone does not
install the CLI, themes, or runtime assets. Publishing and deployment remain explicit,
user-authorized GitHub writes.
...
- Direct Markdown, JSON, explicitly trusted HTML, all document kinds, canonical
  ingestion, project identity, themes, and GitHub Pages publication remain.
