...
test("README keeps installation concise and points detailed setup to one guide", () => {
  ...
  for (const heading of ["### CLI-only", "### CLI plus `planloft-write-doc`"]) {
...
test("setup guide covers human, agent, and CI setup", () => {
  ...
  for (const agent of ["codex", "claude-code", "pi"]) {
    const base = `npx skills add hatim-s/planloft --skill planloft-write-doc`;
...
test("planloft-write-doc permanently requires two-theme documents with a three-option selector", () => {
  const skill = read("skills/planloft-write-doc/SKILL.md");
...
test("consumer surfaces expose the focused skills and no retired aliases", () => {
  ...
  assert.deepEqual(skillDirectories.sort(), ["planloft-customise", "planloft-write-doc"]);
  ...
  assert.match(consumerText, /planloft:customise/);
...
test("migration guide names every breaking installation and runtime contract", () => {
  ...
  for (const retired of ["write-doc", "customize", "customise", "write-plan", "customize-planloft", "save-doc", "planloft-preview", "planloft-copy", "planloft-deploy"]) {
